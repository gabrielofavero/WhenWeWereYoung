...
// ======= LOADERS =======
function _loadSchedule() { // Carrega a programação completa
    ...
    for (let i = 1; dateIndex.getTime() <= END_DATE.getTime(); i++) {
        ...
        let programacaoByDate = _getScheduleByDate(dateIndex);
...
// ======= GETTERS =======
function _getScheduleByDate(date) {
        ...
        for (dataIndex; dataIndex < arrayByDate.length; dataIndex++) {
            ...
            if (startSplit.length > 1) {
                ...
                if (startTime && endTime && startHour >= start && startHour < end) {
                    ...
                    _adaptGastoDuranteAViagem(arrayByDate[i][PROG_INDEX["type"]], valor);
                } else {
                    break;
                }
            }
...
// ======= CONVERTERS =======
function _moneyToFloat(excelMoney) { // Converte o valor do excel para float para as somatórias
    try {
...
function _adaptProgramTitle(data) { // Converte casos especias de descrições. Ignora os que não são necessários
...
function _adaptGastoDuranteAViagem(type, valor) {
